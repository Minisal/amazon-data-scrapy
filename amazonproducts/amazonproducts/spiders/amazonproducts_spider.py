...
class AmazonproductsSpider(Spider):
    ...
    category_URL_list = ["https://www.amazon.com/s/ref=lp_8916179011_pg_" + str(i) +
                         "?rh=n%3A172282%2Cn%3A%2113900851%2Cn%3A%212334089011%2Cn%3A%212334151011%2Cn%3A8916179011&page=" + str(i) +
    ...
    def parse(self, response):
        ...
        for product in product_node_list:
            ...
            try:
                product_url = product.xpath('.//a[contains(@class,"a-link-normal")]/@href').extract_first()
                product_url = "https://www.amazon.com" + product_url
                redis_conn.sadd('Product_URL', product_url)
            except:
                print("=" * 30)
                print("No url for product: ", ASIN)
                product_url = " "

            try:
                int_price = product.xpath('.//span[@class="a-price-whole"]/text()').extract_first()
                float_price = product.xpath('.//span[@class="a-price-fraction"]/text()').extract_first()
                price = int_price + "." + float_price
            except:
                print("=" * 30)
                print("No Price for product: ", ASIN)
                price = " "



            # yield scrapy.Request(product_url,callback=self.more_parse, meta={'cookiejar': numpy.random.randint(1000)})

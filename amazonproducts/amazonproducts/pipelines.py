...
class WriteItemPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        if out != 0:
            ...
            try:
                self.cursor.execute(
                    """INSERT INTO products (asin, category, company, num_review, rating, price, crawl_url, product_url, position) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s)""",
                    (item['ASIN'], item['category'], item['company'], item['num_review'],
                     item['rating'], item['price'], item['crawl_url'], item['product_url'], item['position']))
                ...
                print(item['ASIN'], item['category'], item['company'], item['num_review'], item['rating'],
                      item['price'], item['crawl_url'], item['product_url'], item['position'])
        else:
